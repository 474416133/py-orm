...
async def index(request):
    ...
    **Note: returning html without a template engine like jinja2 is ugly, no way around that.**
    ...
    ctx = dict(
        ...
        content="%s,<p>Success! you've setup a basic aiohttp app.</p>"%"hehll",
    )
...
async def get_account_user(request):
    pool = request.app["pg_engine"]
    ret = await  manager.get_account_user(pool)
    return web.Response(text=json.dumps(ret, cls=manager.RecordEncoder), content_type="application/json")